...
for window_size in x:
    filename = '../results/seq_rf/seq_rf_none_' + str(window_size) + '.csv' # change filename here
...
for window_size in x:
    filename = '../results/seq_rf/seq_rf_balanced_' + str(window_size) + '.csv' # change filename here
    df = pd.read_csv(filename, sep='\t', encoding='utf-8')
...
ax.set_title("Five-fold Cross Validation Accuracy using Optimal Hyper-parameters, RandomForestClassifier()", fontsize=9)
...
print(y2_mean)
#plt.show()
plt.savefig('../report_figures/line_seq_rf.png')
