...
for window_size in x:
    filename = '../results/fm_linear/fm_linear_none_' + str(window_size) + '.csv' # change filename here
...
for window_size in x:
    filename = '../results/fm_linear/fm_linear_balanced_' + str(window_size) + '.csv' # change filename here
    df = pd.read_csv(filename, sep='\t', encoding='utf-8')
...
print(y2_mean)
#plt.show()
plt.savefig('../report_figures/line_fm_linear.png')
